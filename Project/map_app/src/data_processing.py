...
class MapData:
    ...
    def get_k_point_shortest_route(self, user_id):
        # data = self.get_k_closest_locations(user_id, k=10000)
        data = self.get_unvisited_locations(user_id)
        start_loc = self.get_last_visited_location(user_id)
        finish_loc = self.get_second_last_visited_location(user_id)
        data = self.get_k_closest_locations(100, start_loc, data)
        # locations = np.array(data[['lat', 'lng']].values)
        
        # distances = NearestNeighbors(n_neighbors=len(locations)).fit(locations).kneighbors_graph(locations).toarray()
...
data = map_data.get_k_point_shortest_route(2)
print(f"et_k_point_shortest_route took {timeit.default_timer() - start_time} s")
print(data.shape)
print(data.head(10))
